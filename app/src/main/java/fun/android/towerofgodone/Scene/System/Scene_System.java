...
public class Scene_System extends Scene_Base {
    public Scene_System(Context context) {
        ...
        view.findViewById(R.id.button_heritage).setOnClickListener(V->{
            //不存在直接进入
            if(!Fun_File.是否存在(context, "save/heritage_time.txt")){
                new Window_HeriTaGe(context);
                Fun_File.WriteString(Fun_File.getPath(context) + "save/heritage_time.txt", fun.getTime());
                return;
            }
            String time = Fun_File.readString(context, Fun_File.getPath(context) + "save/heritage_time.txt");
            if(Fun_Time.经过多少秒(time) > 3600){
                new Window_HeriTaGe(context);
                Fun_File.WriteString(Fun_File.getPath(context) + "save/heritage_time.txt", fun.getTime());
                return;
    ...
    public void enable_scene(Context context) {
        ...
        Bitmap back = fun.loadBitmapFromAssets(context, "system/back_" + fun.Random(3) + ".png");
